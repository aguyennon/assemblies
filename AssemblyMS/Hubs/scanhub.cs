using AssemblyMS.Services;
using AssemblyMS.Models;
using Microsoft.AspNetCore.SignalR;
using System.Runtime.Versioning;

namespace AssemblyMS.Hubs;
...
public class ScanHub : Hub
{
    ...
    private async Task<ScanResult> ProcessScan(string scannedCode)
    {
        if (string.IsNullOrWhiteSpace(scannedCode))
            ...
            if (rows.Count == 0)
                return new ScanResult(false, $"No Screen (MS) cut rows found in {batchNo}", scannedCode, batchNo, rows);
